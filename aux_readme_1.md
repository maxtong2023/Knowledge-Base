...
Storing facts simply puts any facts received into a list of facts.  Be careful to only put a fact in the list and not just anything (i.e., check that the argument is, in fact, a fact).  Note that we expect the facts to be stored in a list (and __not__ a set) to ensure that they are retrieved in a deterministic order.  This is not critical to the function of the KB but is essential for the autograder that will be used to grade your code submission.
...
`Bindings` is a list of pairs (bindings), where each pair is a variable (e.g., '?X') and a value (e.g., 'red').  Since it is a list, there may be multiple pairs.  There needs to be exactly one binding for each variable.  For example, in asking for '(color ?X red)', there will be only one binding, the one for '?X'.  But the query for '(color ?X ?Y)'' will result in bindings for '?X' and '?Y'.  See test 5 for an example of bindings containing more than one variable.

Since there may be many facts that match a queried fact, `kb_ask` needs to return a list of bindings (in the data structure `ListOfBindings`), or else False if no facts match. `ListOfBindings` is exactly as the name implies, a list of `Bindings`, packaged up in a class with convenient accessors and such.


## Testing

To test your code, we'll create several testing files that will contain a bunch of facts similar to the ones provided.  Each fact will be asserted one by one into the KB. After asserting the facts, a query will be constructed, and the KB will be asked for a fact.  __We also recommend that you make your own testing files__ and that you feel free to discuss them with TA or Peer Mentor. When sharing tests, please provide your rationale for each test, explain what you hope to test with it, and/or describe how you developed the test.

## Sneak-peek of Part 2

In Part 2, you will be extending the KB. You will introduce rules that may also be asserted. Given the rules and facts asserted, you will infer new facts.

__As you implement Part 1, you may want to think ahead to how you would extend the KB to handle rules and inference.__
...
Represents a fact in our knowledge base (KB). Has a statement containing the content of the fact, e.g., `(isa Sorceress Wizard)` and fields tracking which facts/rules in the KB it supports and is supported by.  You don't need to worry about these fields for this assignment.
...
- `supported_by` (`listof Fact|Rule`): Facts/Rules that allow inference of the statement [you don't need to worry about this for this assignment].
...
- `asserted` (`bool`): flag indicating if rule was asserted instead of inferred from other rules/facts in the KB [you don't need to worry about this for this assignment].
- `supported_by` (`listof Fact|Rule`): Facts/Rules that allow inference of the statement [ditto].
